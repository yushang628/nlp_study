...
| `GET` | `/leaderboard` | Agent能力排行榜 |
| `GET` | `/evolution/report` | 进化报告（支持 `?role_type=xxx`） |
| `GET` | `/evolution/version-comparison/{role_type}` | 版本对比 |
| `POST` | `/evolution/analyze` | 手动触发进化分析 |
| `GET` | `/experiences/{role_type}` | 角色历史经验查询 |
...
| 3 | `night_result` | 宣布夜晚死亡结果（含遗言） + 猎人智能开枪结算 |
| 4 | `day_start` | 白天开始，公布昨晚信息 |
| 5 | `speech` | 所有存活玩家依次发言 |
| 6 | `vote` | 全体投票放逐（支持平票PK + 遗言） |
...
每个玩家由一个独立的 AI Agent 驱动，具备以下决策能力：

### 角色专属策略

每个角色（狼人/预言家/女巫/猎人/村民）在系统提示词中注入了专属策略指导：

- **狼人**：伪装、悍跳、协调击杀、保护队友
- **预言家**：合理分享查验、适时跳身份、优先查验可疑玩家
- **女巫**：解药/毒药使用时机、隐藏用药状态
- **猎人**：开枪威慑、基于分析选择目标、防止闷枪
- **村民**：逻辑推理、挡刀策略、有效发言

### 信息隔离

Agent 只能看到自己视野内的信息，不会"开天眼"：
- 善良阵营不知道其他人的身份
- 狼人只能看到队友，不知道谁是神职
- 对话历史按角色权限过滤
...
| 方法 | 触发时机 | 说明 |
|------|----------|------|
| `decide_night_action` | 夜间行动 | 角色专属夜间指南 |
| `decide_speech` | 白天发言 | 含对话历史摘要 |
| `decide_vote` | 投票环节 | 含投票建议分析 |
| `decide_last_words` | 死亡遗言 | 投票/夜间死亡均可触发 |
| `decide_hunter_shot` | 猎人开枪 | AI智能选择目标 |

### 平票PK与遗言

- **平票PK**：得票相同的玩家进行PK发言 → 其他玩家重新投票 → 仍平票随机淘汰
- **遗言机制**：所有死亡（投票出局/夜间被刀）都会触发遗言，遗言内容记录到游戏日志
- **猎人开枪**：猎人死亡时由AI代理智能选择开枪目标，失败时回退到第一个存活玩家

---

## 🧬 自进化Agent循环

系统实现了完整的自进化循环："对局 → 经验沉淀 → 进化分析 → Prompt优化 → 再对局"

### 核心组件

| 组件 | 位置 | 职责 |
|------|------|------|
| `EvolutionAgent` | `agent/evolution_agent.py` | 分析多局经验，提炼进化建议 |
| `EvolutionTracker` | `evaluation/evolution_tracker.py` | 追踪进化效果，记录指标变化 |
| 经验存储 | `memory/experience.py` | 策略标签提取 + 智能检索 + 经验上限控制 |

### 进化流程

1. **对局结束** → SummaryAgent 生成总结 → 经验自动保存到 `memory/experiences/`
2. **经验积累** ≥ 3局 → EvolutionTracker 自动记录进化指标
3. **手动触发** `POST /evolution/analyze` → EvolutionAgent 分析所有角色经验，输出进化建议
4. **应用优化** → 将建议注入下一局游戏的角色Prompt

### 经验智能检索

- **策略标签**：自动从经验中提取 `winning_strategy`、`jump_seer`、`stealth` 等标签
- **局势匹配**：根据当前局势优先匹配相关历史经验
- **经验上限**：每角色最多20条，避免无限增长
...
### 最新版本 v0.4.0 (2026-05-27)

- 自进化Agent循环：EvolutionAgent + EvolutionTracker
- 经验存储增强：策略标签 + 智能检索 + 上限控制
- 新增5个API端点：排行榜、进化报告、版本对比、进化分析、经验查询