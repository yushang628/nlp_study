...
class TestGameSummaryPhase:
    ...
    async def test_run_summaries_called_once(self):
        ...
        for p in engine.game_state.players:
            ...
            p.agent.decide_last_words = AsyncMock(return_value={"content": "我的遗言"})
            p.agent.decide_hunter_shot = AsyncMock(return_value={"target": 1, "reasoning": "test"})
...
class TestHunterSmartDecision:
    """测试猎人智能开枪决策"""

    @pytest.mark.asyncio
    async def test_hunter_uses_ai_decision(self):
        """测试猎人使用AI决策开枪目标"""
        player_names = ["玩家1", "玩家2", "玩家3"]
        role_assignment = {0: "hunter", 1: "werewolf", 2: "villager"}

        engine = await create_game(player_names, role_assignment)
        for p in engine.game_state.players:
            p.agent = MagicMock()
            p.agent.decide_night_action = AsyncMock(
                return_value={"target": 2, "reasoning": "test"}
            )
            p.agent.decide_speech = AsyncMock(return_value={"content": "test"})
            p.agent.decide_vote = AsyncMock(return_value={"target": 1})
            p.agent.decide_last_words = AsyncMock(return_value={"content": "遗言"})
            p.agent.decide_hunter_shot = AsyncMock(
                return_value={"target": 2, "reasoning": "AI选择的target"}
            )

        # 手动模拟猎人死亡
        hunter = engine.game_state.get_player(0)
        hunter.role.is_alive = False
        await engine._handle_hunter_death([0])

        # 验证AI决策被调用
        hunter.agent.decide_hunter_shot.assert_called_once()

    @pytest.mark.asyncio
    async def test_hunter_fallback_when_ai_fails(self):
        """测试AI决策失败时回退到第一个存活玩家"""
        player_names = ["玩家1", "玩家2", "玩家3"]
        role_assignment = {0: "hunter", 1: "werewolf", 2: "villager"}
...
class TestLastWords:
    ...
    @pytest.mark.asyncio
    async def test_eliminate_player_with_last_words(self):
        """测试处决玩家时记录遗言"""
        player_names = ["玩家1", "玩家2", "玩家3"]
        role_assignment = {0: "werewolf", 1: "villager", 2: "villager"}
        ...
        for p in engine.game_state.players:
            p.agent = MagicMock()
            p.agent.decide_last_words = AsyncMock(
                return_value={"content": "我是好人，请相信我"}
            )

        await engine._eliminate_player(1, "vote")

        player = engine.game_state.get_player(1)
        assert not player.is_alive
        assert len(engine.game_state.last_words) >= 1
        assert "我是好人" in engine.game_state.last_words[0]["words"]

    @pytest.mark.asyncio
    async def test_night_death_last_words(self):
        """测试夜间死亡也有遗言"""
        player_names = ["玩家1", "玩家2", "玩家3"]
        role_assignment = {0: "werewolf", 1: "villager", 2: "villager"}
        ...
        for p in engine.game_state.players:
            ...
            p.agent.decide_vote = AsyncMock(return_value={"target": 0})
            p.agent.decide_last_words = AsyncMock(
                return_value={"content": "夜间遗言"}
            )

        engine.game_state.night_deaths = [1]
        await engine._announce_night_deaths()

        assert len(engine.game_state.last_words) >= 1


class TestTieBreakerPK:
    """测试平票PK机制"""

    @pytest.mark.asyncio
    async def test_tie_breaker_pk_eliminates_one(self):
        """测试平票PK后淘汰一个玩家"""
        player_names = ["玩家1", "玩家2", "玩家3", "玩家4"]
        role_assignment = {0: "werewolf", 1: "villager", 2: "villager", 3: "villager"}
        ...
        for p in engine.game_state.players:
            ...
            p.agent.decide_vote = AsyncMock(return_value={"target": 0})
            p.agent.decide_last_words = AsyncMock(return_value={"content": "遗言"})
            p.agent.decide_hunter_shot = AsyncMock(return_value={"target": 1})

        # 直接测试PK函数
        initial_alive = len(engine.game_state.get_alive_players())
        await engine._tie_breaker_pk([1, 2])
        final_alive = len(engine.game_state.get_alive_players())
        assert final_alive < initial_alive

    @pytest.mark.asyncio
    async def test_tie_breaker_pk_with_no_voters(self):
        """测试无人投票的PK"""
        player_names = ["玩家1", "玩家2"]
        role_assignment = {0: "werewolf", 1: "villager"}
        ...
        for p in engine.game_state.players:
            p.agent = MagicMock()
            p.agent.decide_speech = AsyncMock(return_value={"content": "test"})
            p.agent.decide_vote = AsyncMock(return_value={"target": None})
            p.agent.decide_last_words = AsyncMock(return_value={"content": "遗言"})

        await engine._tie_breaker_pk([0, 1])
        # 两人PK无人投票，应随机淘汰一个
        alive = engine.game_state.get_alive_players()
        assert len(alive) <= 2
...
class TestDialogueSummary:
    ...
    def test_empty_dialogues(self):
        ...
        result = PlayerAgent._build_dialogue_summary([])
        assert "暂无" in result

    def test_speech_summary(self):
        """测试发言摘要"""
        from agent.player_agent import PlayerAgent
        dialogues = [
            {"day": 1, "player_name": "玩家1", "player_id": 0, "action": "speech", "content": "我觉得玩家2很可疑"},
            {"day": 1, "player_name": "玩家2", "player_id": 1, "action": "vote", "target": 0},
        ]
        result = PlayerAgent._build_dialogue_summary(dialogues)
    ...
    def test_dialogue_truncation(self):
        ...
        from agent.player_agent import PlayerAgent
        dialogues = [
            {"day": 1, "player_name": f"玩家{i}", "player_id": i, "action": "speech", "content": f"发言{i}"}
            for i in range(30)
        ]
        result = PlayerAgent._build_dialogue_summary(dialogues)
        # 只取最后20条，所以应该不包含"发言0"
        assert "发言0" not in result
        assert "发言29" in result


class TestRoleStrategies:
    """测试角色专属策略"""

    def test_all_roles_have_strategies(self):
        """测试所有角色都有专属策略"""
        from agent.player_agent import ROLE_STRATEGIES
        required_roles = ["werewolf", "seer", "witch", "hunter", "villager"]
        for role in required_roles:
            assert role in ROLE_STRATEGIES
            assert len(ROLE_STRATEGIES[role]) > 20

    def test_role_strategy_in_instructions(self):
        """测试角色策略被包含在指令中"""
        from agent.player_agent import PlayerAgent
        with patch('agent.player_agent.config') as mock_config:
            ...
            assert "伪装" in instructions or "狼人" in instructions


class TestNightActionGuide:
    """测试夜间行动指南"""

    def test_werewolf_guide(self):
        """测试狼人夜间指南"""
        from agent.player_agent import PlayerAgent
        with patch('agent.player_agent.config') as mock_config:
            mock_config.default_model = "gpt-4"
            agent = PlayerAgent(
                player_id=0, role_name="狼人",
                private_context={}, camp="evil", role_type="werewolf",
            )
            guide = agent._get_night_action_guide({})
            assert "击杀" in guide

    def test_seer_guide(self):
        """测试预言家夜间指南"""
        from agent.player_agent import PlayerAgent
        with patch('agent.player_agent.config') as mock_config:
            mock_config.default_model = "gpt-4"
            agent = PlayerAgent(
                player_id=0, role_name="预言家",
                private_context={}, camp="good", role_type="seer",
            )
            guide = agent._get_night_action_guide({})
            assert "查验" in guide

    def test_witch_guide(self):
        """测试女巫夜间指南"""
        from agent.player_agent import PlayerAgent
        with patch('agent.player_agent.config') as mock_config:
            mock_config.default_model = "gpt-4"
            agent = PlayerAgent(
                player_id=0, role_name="女巫",
                private_context={}, camp="good", role_type="witch",
            )
            guide = agent._get_night_action_guide({"tonight_death": 2, "has_heal": True, "has_poison": True})
            assert "解药" in guide
            assert "毒药" in guide