...
class TestPlayerAgent:
    ...
    def test_build_night_prompt(self, mock_agent_config, sample_private_context):
        ...
        prompt = agent._build_night_prompt({"day_number": 1, "phase": "night"})
        assert "夜间" in prompt or "夜晚" in prompt or "夜" in prompt
        assert "击杀" in prompt or "目标" in prompt
    ...
    def test_build_speech_prompt(self, mock_agent_config, sample_private_context):
        ...
        prompt = agent._build_speech_prompt({"day_number": 1, "phase": "speech"})
        assert "发言" in prompt
        assert "JSON" in prompt or "json" in prompt or "action" in prompt
    ...
    def test_build_vote_prompt(self, mock_agent_config, sample_private_context):
        ...
        prompt = agent._build_vote_prompt({"day_number": 1, "phase": "vote"})
        assert "投票" in prompt
...
class TestDecisionStyleIntegration:
    ...
    def test_random_style_prompt(self, mock_agent_config, private_context):
        ...
        assert "随机" in vote_prompt or "直觉" in vote_prompt or "弃票" in vote_prompt
    ...
    def test_balanced_style_prompt(self, mock_agent_config, private_context):
        ...
        assert "分析" in speech_prompt or "逻辑" in speech_prompt
