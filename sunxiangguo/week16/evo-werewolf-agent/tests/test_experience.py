...
class TestStrategyTags:
    ...
    def test_winning_strategy_tag(self):
        exp = {"is_winner": True, "role_type": "villager", "strategies": "", "summary": ""}
        tags = _extract_strategy_tags(exp)
        assert "winning_strategy" in tags

    def test_failure_case_tag(self):
        exp = {"is_winner": False, "role_type": "villager", "strategies": "", "summary": ""}
        tags = _extract_strategy_tags(exp)
        assert "failure_case" in tags

    def test_werewolf_jump_seer_tag(self):
        exp = {"is_winner": True, "role_type": "werewolf", "strategies": "悍跳预言家", "summary": ""}
        tags = _extract_strategy_tags(exp)
        assert "jump_seer" in tags

    def test_werewolf_stealth_tag(self):
        exp = {"is_winner": True, "role_type": "werewolf", "strategies": "低调隐身", "summary": ""}
        tags = _extract_strategy_tags(exp)
        assert "stealth" in tags

    def test_seer_hide_identity_tag(self):
        exp = {"is_winner": True, "role_type": "seer", "strategies": "暗查隐藏身份", "summary": ""}
        tags = _extract_strategy_tags(exp)
        assert "hide_identity" in tags

    def test_witch_use_poison_tag(self):
        exp = {"is_winner": False, "role_type": "witch", "strategies": "使用毒药毒杀", "summary": ""}
        tags = _extract_strategy_tags(exp)
        assert "use_poison" in tags


class TestMaxExperiences:
    """测试经验上限控制"""
    ...
    def _cleanup(self):
        test_file = os.path.join(EXPERIENCES_DIR, "test_max.json")
        if os.path.exists(test_file):
            os.remove(test_file)

    def test_experience_limit(self):
        """测试经验上限不超过MAX_EXPERIENCES"""
        for i in range(MAX_EXPERIENCES + 5):
            save_experience("test_max", {"game_id": f"g{i}", "summary": f"经验{i}"})
        exps = load_experiences("test_max")
        assert len(exps) <= MAX_EXPERIENCES

    def test_latest_experiences_kept(self):
        """测试保留的是最新经验"""
        for i in range(MAX_EXPERIENCES + 5):
            save_experience("test_max", {"game_id": f"g{i}", "summary": f"经验{i}"})
        exps = load_experiences("test_max")
        # 最新的经验应该在列表中
        last = exps[-1]
        assert last["game_id"] == f"g{MAX_EXPERIENCES + 4}"
...
class TestSmartRetrieval:
    ...
    def _cleanup(self):
        test_file = os.path.join(EXPERIENCES_DIR, "test_smart.json")
        if os.path.exists(test_file):
            os.remove(test_file)

    def test_find_relevant_experiences(self):
        """测试相关经验检索"""
        exps = [
            {"summary": "狼人悍跳预言家成功", "strategies": "悍跳", "mistakes": "", "lessons": ""},
            {"summary": "普通村民投票", "strategies": "", "mistakes": "", "lessons": ""},
            {"summary": "预言家被狼人杀害", "strategies": "验人", "mistakes": "没跳", "lessons": ""},
        ]
        result = _find_relevant_experiences(exps, "狼人悍跳")
        assert len(result) >= 1

    def test_situation_based_prompt(self):
        """测试带局势的经验提示词"""
        save_experience("test_smart", {
            "summary": "狼人被预言家查验出局",
            "strategies": "悍跳",
            "mistakes": "太早暴露",
            "is_winner": False,
        ...
        prompt = get_experience_prompt("test_smart", situation="狼人悍跳预言家")
        assert "过往游戏经验" in prompt


class TestClearAndGetAll:
    """测试清空和获取所有经验"""
    ...
    def test_clear_specific_role(self):
        save_experience("werewolf", {"summary": "test"})
        clear_experiences("werewolf")
        assert load_experiences("werewolf") == []

    def test_clear_all_roles(self):
        save_experience("werewolf", {"summary": "test1"})
        save_experience("seer", {"summary": "test2"})
        clear_experiences()
        assert load_experiences("werewolf") == []
        assert load_experiences("seer") == []

    def test_get_all_role_experiences(self):
        # 清空先
        clear_experiences()
        save_experience("werewolf", {"summary": "wolf1"})
        save_experience("seer", {"summary": "seer1"})
        all_exps = get_all_role_experiences()
        assert "werewolf" in all_exps
        assert "seer" in all_exps
        assert len(all_exps["werewolf"]) == 1
        clear_experiences()
